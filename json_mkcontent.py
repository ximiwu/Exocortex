...
def merge_para_with_text(para_block, formula_enable=True, img_buket_path='', md_image_path_resolver=None):
    para_text = ''
        ...
        for j, span in enumerate(line['spans']):
            ...
            if span_type == ContentType.TEXT:
                ...
                if formula_enable:
                    ...
                    if image_path:
                        if md_image_path_resolver:
                            resolved_path = md_image_path_resolver(span)
                            if resolved_path:
                                image_path = resolved_path
                        content = f"![]({img_buket_path}/{image_path})"
...
def mk_blocks_to_markdown(para_blocks, make_mode, formula_enable, table_enable, img_buket_path='', md_image_path_resolver=None):
    page_markdown = []
    for para_block in para_blocks:
        ...
        if para_type in [BlockType.TEXT, BlockType.INTERLINE_EQUATION, BlockType.PHONETIC, BlockType.REF_TEXT]:
            para_text = merge_para_with_text(
                ...
                md_image_path_resolver=md_image_path_resolver,
            )
            ...
            para_text = f'{"#" * title_level} {merge_para_with_text(para_block, formula_enable=formula_enable, img_buket_path=img_buket_path, md_image_path_resolver=md_image_path_resolver)}'
                                    ...
                                    if span['type'] == ContentType.IMAGE:
                                        ...
                                        if image_path:
                                            if md_image_path_resolver:
                                                ...
                                                if resolved_path:
                                                    image_path = resolved_path
                                            para_text += f"![]({img_buket_path}/{image_path})"
...
def union_make(pdf_info_dict: list,
               ...
               md_image_writer=None,
               md_image_dpi: int | None = None,
               md_image_max_side: int | None = None,
               ):

    formula_enable = get_formula_enable(os.getenv('MINERU_VLM_FORMULA_ENABLE', 'True').lower() == 'true')
    table_enable = get_table_enable(os.getenv('MINERU_VLM_TABLE_ENABLE', 'True').lower() == 'true')

    def _safe_int_env(env_name: str, default_value: int) -> int:
        ...
        try:
            ...
            logger.warning(f"Invalid {env_name}={env_val!r}, fallback to {default_value}")
            return default_value

    enable_md_high_res_images = (
        make_mode in [MakeMode.MM_MD, MakeMode.NLP_MD]
        and pdf_bytes is not None
        and md_image_writer is not None
    )

    if md_image_dpi is None:
        md_image_dpi = _safe_int_env("MINERU_VLM_MD_IMAGE_DPI", 1000)
    if md_image_max_side is None:
        md_image_max_side = _safe_int_env("MINERU_VLM_MD_IMAGE_MAX_SIDE", 6000)

    pdf_doc = None
    page_render_cache = {"page_idx": None, "pil_img": None, "scale": None}
    md_image_cache: dict[tuple, str] = {}

    if enable_md_high_res_images:
        try:
            import pypdfium2 as pdfium  # type: ignore
            pdf_doc = pdfium.PdfDocument(pdf_bytes)
        except Exception as e:
            logger.warning(f"Failed to open PDF for md high-res rendering, fallback to original crops: {e}")
            enable_md_high_res_images = False

    def _resolve_md_image_path(span: dict, page_idx: int) -> str:
        image_path = span.get('image_path', '')
        bbox = span.get('bbox')
        if not enable_md_high_res_images or not image_path or not bbox or pdf_doc is None:
            return image_path

        if not isinstance(bbox, (list, tuple)) or len(bbox) != 4:
            return image_path

        cache_key = (page_idx, span.get('type'), tuple(bbox))
        cached = md_image_cache.get(cache_key)
        if cached:
            return cached

        try:
            if page_render_cache["page_idx"] != page_idx:
                page = pdf_doc[page_idx]
                high_res_scale = md_image_dpi / 72
                page_w, page_h = page.get_size()
                long_side = max(page_w, page_h)
                if md_image_max_side and long_side * high_res_scale > md_image_max_side:
                    high_res_scale = md_image_max_side / long_side

                bitmap = page.render(scale=high_res_scale)  # type: ignore
                ...
                page_render_cache.update({"page_idx": page_idx, "pil_img": pil_img, "scale": high_res_scale})

            from mineru.utils.pdf_image_tools import cut_image

            md_return_path = f"md_highres/{span.get('type', 'unknown')}/dpi{md_image_dpi}/max{md_image_max_side}"
            md_image_path = cut_image(
                tuple(bbox),
                page_idx,
                page_render_cache["pil_img"],
                return_path=md_return_path,
                image_writer=md_image_writer,
                scale=page_render_cache["scale"],
            )
            md_image_cache[cache_key] = md_image_path
            return md_image_path
        except Exception as e:
            logger.warning(f"Failed to re-render md high-res crop for page {page_idx}: {e}")
            return image_path
        ...
        for page_info in pdf_info_dict:
            ...
            if make_mode in [MakeMode.MM_MD, MakeMode.NLP_MD]:
                if not paras_of_layout:
                    continue
                md_image_path_resolver = None
                if enable_md_high_res_images:
                    md_image_path_resolver = lambda span, _page_idx=page_idx: _resolve_md_image_path(span, _page_idx)
